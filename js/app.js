...
// 新增：功能开关状态（默认关闭）
let settings = {
    speakEnabled: true,  // 语音朗读开关，默认打开
    soundEnabled: true   // 提示音效开关，默认打开
};
...
function initSpeechSynthesis() {
    ...
    setTimeout(() => {
        ...
        if (!russianVoice) {
            console.warn('未找到俄语语音包');
            feedback.textContent = "未找到俄语语音包，请检查系统设置";
        } else {
            console.log('使用俄语语音:', russianVoice.name);
        }
    }, 1000);
}
...
function showWordShowcasePage() {
    ...
    wordCardsContainer.querySelectorAll('.showcase-speak-btn').forEach(btn => {
        btn.addEventListener('click', function(e) {
            ...
            let voice = russianVoice;
            if (!voice) {
                // 兼容部分浏览器未及时加载voice
                const voices = window.speechSynthesis.getVoices();
                voice = voices.find(v => v.lang === 'ru-RU' || v.lang?.startsWith('ru-') || v.name?.toLowerCase().includes('russian'));
            }
            if (!voice) return;
            try {
